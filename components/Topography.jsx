...
const Topography = ({
  ...
  useEffect(() => {
    ...
    const mobileOrCoarse = window.matchMedia("(max-width: 767px), (pointer: coarse)");
    const connection = navigator.connection;
    let resizeFrame = 0;

    const updateCapability = () => {
      const constrainedConnection = Boolean(connection?.saveData || /(^|-)2g$/.test(connection?.effectiveType ?? ""));
      setEffectsEnabled(!reducedMotion.matches && !mobileOrCoarse.matches && !constrainedConnection);
    };
    const onResize = () => {
  ...
  useEffect(() => {
    ...
    if (!container || !effectsEnabled) return;
    ...
    const onPointerMove = (event) => {
      ...
      targetMouse[0] = (event.clientX - bounds.left) / Math.max(bounds.width, 1);
      targetMouse[1] = 1.0 - (event.clientY - bounds.top) / Math.max(bounds.height, 1);
      mouseActiveTarget = 1;
    };
    const deactivateMouse = () => { mouseActiveTarget = 0; };
    ...
    const ctrlArrays = [program.uniforms.uCtrlA.value, program.uniforms.uCtrlB.value, program.uniforms.uCtrlC.value, program.uniforms.uCtrlD.value];
    let raf = 0;
    let isVisible = true;
    ...
    const loop = (timeStamp) => {
      ...
      for (let group = 0; group < 4; group++) {
        ...
        for (let index = 0; index < 4; index++) {
          ...
          array[index] = uniforms.uMorphAmount.value * Math.sin(time * uniforms.uSpeed.value * Math.sin(controlIndex * uniforms.uMorphSpeed.value) + controlIndex);
        }
      }
  ...
  }, [lowColor, midColor, highColor, speed, morphAmount, morphSpeed, bands, thickness, scale, pixelSize, glow, colorMode, contrast, brightness, fillBands, opacity, grain, grainIntensity, mouseInteraction, mouseRadius, mouseStrength]);

  return <div ref={containerRef} className={`topography-container topography-fallback ${className}`.trim()} />;
};
